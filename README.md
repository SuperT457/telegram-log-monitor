...
- The script uses `watchdog` module to detect file changes (event-driven) to avoid poolling and improve performance and energy efficiency;
...
- At the end of the loop, telegram APIs are used to alert the user with a message containg all the latest accesses;
...
## Latest upgrades 
- Integrated `last_pos` and `ip_cache` in a unique class
